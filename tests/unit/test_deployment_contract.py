...
def test_compose_build_has_a_real_dockerfile_and_runtime_services() -> None:
    ...
    assert "POSTGRES_PASSWORD_FILE: /run/secrets/database_credential" in compose
    assert "database_credential:" in compose
    assert "DMI_DATABASE_SECRET_FILE:?Set DMI_DATABASE_SECRET_FILE to a protected file" in compose
...
def test_example_environment_is_safe_and_complete_enough_to_boot_after_setup() -> None:
    ...
    assert "DMI_DATABASE_SECRET_FILE=" in example
    assert (
        "DATABASE_URL=postgresql+asyncpg://postgres:replace-me@postgres:5432/dream_motif" in example
    )
...
def test_ci_verifies_compose_and_non_root_container_contract() -> None:
    ...
    assert "Refuse missing database credential file" in workflow
    assert "docker compose config succeeded without DMI_DATABASE_SECRET_FILE" in workflow
    assert "DMI_DATABASE_SECRET_FILE: .compose-ci-db-credential" in workflow
