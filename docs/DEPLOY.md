...
Put the generated value in `SECRET_KEY`. Create a separate random database credential in a
mode-0600 file, set `DMI_DATABASE_SECRET_FILE` to its absolute host path, and set `DATABASE_URL`
to the Compose-network DSN (`postgres` is the hostname). Configure Telegram/provider credentials,
then:
...
There is no database-credential fallback: Compose refuses to render until
`DMI_DATABASE_SECRET_FILE` names a protected file. PostgreSQL reads that file through a Compose
secret; application services receive their database DSN only from the ignored `.env` file.
PostgreSQL, Redis and the API publish to
