...
Status: Implemented — Test 9 regressions closed; full-text follow-up bypass added
...
```text
Make full dream text replay deterministic. In app/assistant/chat.py detect explicit full-text dream requests. After get_dream or a single search_dreams_by_title full-dream result, format the Date/Title/Text payload directly for the user and return it before the final LLM generation step. Keep normal analysis/search flows unchanged. Add tests that a long dream result is returned with its final line and that client.messages.create is called only once.
```
