...
`DMI_DATABASE_SECRET_FILE` обязателен: Compose читает credential PostgreSQL из protected host
file, а `DATABASE_URL` получает только из ignored `.env`. PostgreSQL, Redis и API по умолчанию
публикуются только на `127.0.0.1`; изменение bind-address — отдельное операторское решение,
