...
- remaining: publish this slice to `codex/end-to-end-hardening`, then watch CI. PostgreSQL/live Telegram/Whisper canaries remain future work.
- next step: publish this commit, sync local history, then continue Phase B only if another bounded voice recovery gap is found; otherwise move to the Phase C Google Docs canary script/docs slice.
